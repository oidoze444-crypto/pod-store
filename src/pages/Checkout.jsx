...
import { ordersApi, productsApi, settingsApi } from '../components/mysqlApi';
import { CartProvider, useCart } from '../components/store/CartContext';
import { ArrowLeft, MapPin, User, Loader2, Send, Minus, Plus, Trash2, ShoppingBag } from 'lucide-react';
...
function CheckoutContent() {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.number.trim()) { toast.error('Preencha o número!'); return; }
    if (items.length === 0) { toast.error('Carrinho vazio!'); return; }
    if (settings.min_order_value && subtotal < settings.min_order_value) {
      toast.error(`Pedido mínimo de R$ ${settings.min_order_value.toFixed(2).replace('.', ',')}`);
      return;
    }
    ...
    const address = `${form.street}, ${form.number}${form.complement ? ', ' + form.complement : ''} - ${form.neighborhood}, ${form.city}/${form.state} - CEP: ${form.cep}${form.reference ? '\nReferência: ' + form.reference : ''}`;

    const message = `🛒 *Novo Pedido*\n\n👤 *Cliente:* ${form.name}\n\n📍 *Endereço:*\n${address}\n\n📦 *Itens:*\n${itemsList}${deliveryFee > 0 ? `\n\n🚚 Taxa de entrega: R$ ${deliveryFee.toFixed(2).replace('.', ',')}` : ''}\n\n💰 *Total: R$ ${total.toFixed(2).replace('.', ',')}*`;
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="sticky top-0 z-10 bg-white border-b">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={() => navigate(createPageUrl('Store'))} className="p-2 hover:bg-gray-100 rounded-xl">
            ...
            {items.map(item => (
              <div key={item.key} className="flex items-center gap-3 pb-3 border-b last:border-0">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => updateQuantity(item.key, item.quantity - 1)} className="w-6 h-6 rounded-full border flex items-center justify-center">
        ...
        <section className="bg-white rounded-2xl p-4 sm:p-5">
          ...
          <div className="space-y-3">
            <div className="relative">
              ...
              />
              {loadingCep && <Loader2 className="absolute right-3 top-3 w-4 h-4 animate-spin text-emerald-600" />}
            </div>
            <input
              type="text"
              placeholder="Rua"
