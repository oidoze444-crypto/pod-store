  ...
  return (
    <AdminLayout title="Sabores">
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm">
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input type="text" value={editing.name} onChange={e => setEditing(p => ({ ...p, name: e.target.value }))} placeholder="Ex: Morango" className="w-full px-4 py-2.5 bg-gray-50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
      ...
      <div className="space-y-2">
        {flavors.map(flavor => (
          <div key={flavor.id} className="bg-white rounded-2xl px-5 py-4 shadow-sm border border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Droplets className="w-5 h-5 text-emerald-500" />
              <span className="font-medium text-sm">{flavor.name}</span>
              {!flavor.is_active && <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">Inativo</span>}
            </div>
            <div className="flex gap-1">
              <button onClick={() => setEditing({ ...flavor })} className="p-2 hover:bg-gray-100 rounded-xl">
                <Pencil className="w-4 h-4 text-gray-500" />
              </button>
              <button onClick={() => { if (confirm('Excluir?')) deleteMutation.mutate(flavor.id); }} className="p-2 hover:bg-red-50 rounded-xl">
